        ...
        <div class="main_profile_form">
          ...
          <form class="row" @submit.prevent="login">
            ...
            <div class="w-100 d-flex justify-content-center">
              <div class="col-lg-4 col-12">
                <nuxt-link class="btn_to_go_reg" :to="localePath('/auth/user/register')">
                  register as user
                </nuxt-link>
              </div>
            </div>
...
export default {
  ...
  methods: {
    async login() {
      ...
      formData.append("role", this.form.password);
      await this.$Api.auth.userLogin(formData).then((res) => {
    ...
    async verifyCodeFun(){
      ...
      await this.$Api.auth.userVerifyEmailCode(formData).then((res) => {
        this.isLoading = false;
